...
Page({
  ...
  _time() {
    ...
    let pageSize = 10
    let currPage = 1
    getSystem("https://www.jnxsdwmsj.com/api/PersonalInterface/getServiceTime", { userId, sessionId, cityCode, token, pageSize, currPage }, res => {
      if (res.data.code == 0) {
        this.setData({
  ...
  _jifen() {
    ...
    let pageSize = 10
    let currPage = 1
    getSystem("https://www.jnxsdwmsj.com/api/PersonalInterface/getScoreList", { userId, sessionId, cityCode, token, pageSize, currPage }, res => {
      if (res.data.code == 0) {
        this.setData({
          list: res.data.data.list,
          idx: 1
